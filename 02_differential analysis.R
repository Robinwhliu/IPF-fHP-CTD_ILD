...
exp1 <- exp[,pd_GSE48149[pd_GSE48149$Group != "IPF",] %>% rownames()]
group_list1 = unlist(lapply(pd_GSE48149[pd_GSE48149$Group != "IPF",]$Group, 
                           function(x) ifelse(x == "NL", "Control", "Case")))
exp2 <- exp[,pd_GSE48149[pd_GSE48149$Group != "SSc PF",] %>% rownames()]
group_list2 = unlist(lapply(pd_GSE48149[pd_GSE48149$Group != "SSc PF",]$Group, 
                            function(x) ifelse(x == "NL", "Control", "Case")))
deg_SSc = differ(exp1, group_list1)
res_SSc = data.frame(row.names = fd_GSE48149[rownames(deg_SSc),]$Symbol, 
                 FoldChange = deg_SSc$logFC, adjusted_P.value = deg_SSc$adj.P.Val)
deg_IPF = differ(exp2, group_list2)
res_IPF = data.frame(row.names = fd_GSE48149[rownames(deg_IPF),]$Symbol, 
                     FoldChange = deg_IPF$logFC, adjusted_P.value = deg_IPF$adj.P.Val)
V_SSc = EnhancedVolcano(res_SSc,
                     lab = rownames(res_SSc),
                     x = 'FoldChange',
                     y = 'adjusted_P.value',
                     title = "Volcano plot of SSc_PF",
                     subtitle = NULL,
                     caption = NULL,
...
res_IPF %>% rownames() %>% grep(pattern = "March 4", x = .)
res_IPF = res_IPF[-431,]
V_IPF = EnhancedVolcano(res_IPF,
                        lab = rownames(res_IPF),
                        x = 'FoldChange',
                        y = 'adjusted_P.value',
...
#      file = "exp.Rdata")
# save(res_CHP, res_SSc, res_IPF3, res_IPF, res_CTD, file = "res.Rdata")
load(file = "C:/Users/LWH/Downloads/bioinfo/IPF&CHD&SSc/res.Rdata")
library(xlsx)
res_IPF_n <- res_IPF3
res_IPF_n$FoldChange <- (res_IPF_n$FoldChange)/2
write.xlsx(res_CHP,file = "Original Data.xlsx",sheetName = "DEGs_CHP",row.names = F)
write.xlsx(res_IPF_n,file = "Original Data.xlsx",sheetName = "DEGs_IPF",
           row.names = T,append = T)
write.xlsx(res_CTD,file = "Original Data.xlsx",sheetName = "DEGs_CTD-ILD",
           row.names = T,append = T)
...
gene_inter_down <- data.frame(hgnc_symbol = genename, 
                            ...
                            logFC_CHP = res_CHP[genename,]$FoldChange,
                            `logFC_CTD-ILD` = res_CTD[genename,]$FoldChange)
save(gene_up, gene_down, gene_id, degs1, degs2, degs3, fd_GSE150910, fd_GSE175457, fd_GSE48149, 
     fd_GSE199152, file = "venndata.Rdata")
